...
extern bool ShowArrows = true; // Option to show buy/sell arrows
//---- indicator buffers
double LineBuffer[];
double UpBuffer[];
double DnBuffer[];
...
double StepMACalc(bool HL, double Size, int k)
{
    ...
    if (counted_bars == 0)
    {
        smax[limit + 1] = smax[limit];
        smin[limit + 1] = smin[limit];
        trend[limit + 1] = 0;
    }
    ...
    for (shift = limit; shift >= 0; shift--)
    {
        ...
        else if (ColorMode == 2)
        {
            if (trend[shift] > 0)
            {
                ...
                if (trend[shift + 1] < 0)
                    UpBuffer[shift + 1] = DnBuffer[shift + 1];
                DnBuffer[shift] = EMPTY_VALUE;
            }
            if (trend[shift] < 0)
            {
                ...
                if (trend[shift + 1] > 0)
                    DnBuffer[shift + 1] = UpBuffer[shift + 1];
                UpBuffer[shift] = EMPTY_VALUE;
            }
        }
        else
        ...
        // Check for StepMA crossover with close price
        if (ShowArrows)
        {
            if (!isBuy && Close[shift] > StepMA && Close[shift + 1] <= StepMA)
            {
                BuyArrowBuffer[shift] = Low[shift] - 2 * Point;
                SellArrowBuffer[shift] = EMPTY_VALUE;
                isBuy=true;
                isSell=false;
            }
            else if (Close[shift] < StepMA && Close[shift + 1] >= StepMA)
            {
                SellArrowBuffer[shift] = High[shift] + 2 * Point;
